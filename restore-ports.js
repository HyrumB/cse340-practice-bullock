    ...
    getOSCommands() {
        const commands = {
            win32: {
                ...
                processInfo: (pid) => `tasklist /FI "PID eq ${pid}" /FO LIST`
            },
            darwin: {
                check: (port) => `lsof -i :${port} -sTCP:LISTEN -t`,
                getPid: (output) => output.trim(),
                kill: (pid) => `kill -9 ${pid}`,
                processInfo: (pid) => `ps -p ${pid} -o comm=`
            },
            linux: {
                check: (port) => `ss -lptn 'sport = :${port}'`,
                getPid: (output) => {
