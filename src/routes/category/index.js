...
import { addNewGame, getClassifications, getGamesByClassification, getGameById, updateGame, deleteGame } from '../../models/index.js';
...
router.get('/delete/:id', async (req, res) => {
    const game = await getGameById(req.params.id);
    res.render('category/delete', { title: 'Delete Game', game });
});

// Delete route to accept game deletion
router.post('/delete/:id', async (req, res) => {
    const gameId = req.params.id;
    const classification_id = req.body.classification_id;
    const newClassification_id = req.body.newClassification_id;
    await deleteGame(gameId, newClassification_id);
    res.redirect(`/category/view/${classification_id}`);
});
